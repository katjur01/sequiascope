...
box::use(
  rhino,
  shiny[h1,h2,h3,bootstrapPage,div,moduleServer,NS,renderUI,tags,uiOutput,icon,observeEvent,observe,reactive,isTruthy,onFlushed,
...
# 
#     upload_confirmed <- reactiveVal(FALSE)
#     final_selection_paths <- reactiveVal(NULL)
# 
#     session$onFlushed(function(){
#       session$sendCustomMessage("lockMenuExcept", list(
#         menuId     = ns("navbarMenu"),
#         allowValue = ns("upload_data"),  # povolený tab na start
#         lock       = TRUE
#       ))
#     }, once = TRUE)
# 
#     observeEvent(input$navbarMenu, {
#       if (!isTRUE(upload_confirmed()) && input$navbarMenu != ns("upload_data")) {
#         updateNavbarTabs(session, "navbarMenu", selected = ns("upload_data"))
#       }
#     }, ignoreInit = TRUE)
# 
#     observeEvent(upload$confirmed_paths(), {
#       paths <- upload$confirmed_paths()
#       if (is.null(paths) || !is.data.frame(paths) || nrow(paths) == 0) return()
# 
#       final_selection_paths(paths)
#       upload_confirmed(TRUE)
# 
#       session$sendCustomMessage("lockMenuExcept", list(
#         menuId     = ns("navbarMenu"),
#         allowValue = ns("upload_data"),
#         lock       = FALSE
#       ))
